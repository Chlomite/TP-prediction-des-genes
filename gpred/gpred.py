...
__author__ = "Chlomite Cohen"
__copyright__ = "Universite Paris Diderot"
__credits__ = ["Chlomite Cohen"]
__license__ = "GPL"
__version__ = "1.0.0"
__maintainer__ = "Chlomite Cohen"
__email__ = "chlomitecohen@gmail.com"
...
def read_fasta(fasta_file: Path) -> str:
    ...
    try:
        ...
        print(f"Le fichier {fasta_file} n'a pas été trouvé.")
        return None
...
def find_stop(stop_regex: Pattern, sequence: str, start: int) -> Union[int, None]:
    ...
    for i in range(start, len(sequence) - 2, 3):
        ...
        if i % 3 == reading_frame and stop_regex.search(codon):
            return i
    return None
...
def has_shine_dalgarno(
    ...
    :param max_shine_dalgarno_distance: (int) Maximum distance of
    the shine dalgarno to the start position
...
def predict_genes(
    ...
    while len(sequence) - position_courante >= min_gap:
        position_courante = find_start(
            start_regex, sequence, position_courante, len(sequence)
        )
            ...
            if stop_position is not None:
                ...
                if gene_length >= min_gene_len:
                    if has_shine_dalgarno(
                        ...
                        gene = [position_courante + 1, stop_position + 3]
                        genes.append(gene)
